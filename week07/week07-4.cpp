...
public:
    bool isRobotBounded(string instructions) {
        ...
        /// cout << instructions; 這是 debug 用的
        int x = 0, y = 0; /// x 座標, y 座標 一開始要在原點上(0, 0)
        int d = 0; /// d : 方向 direction 0:北 1:東 2:南 3:西
        /// 右轉 d = (d + 1) % 4 取 4 的餘數
        /// 左轉 d = (d - 1 + 4) % 4 竟然可以倒過來, 太神奇了 !
        int dx[4] = {0, 1, 0, -1}; /// 前進一格時，會走 x += dx[d] 及 y += dy[d]
        int dy[4] = {1, 0, -1, 0}; /// 這兩行, 是「地圖20模擬的精華, 決定前進多少」

        for(char c : instructions) { /// 依照字母的指令, 一次做 1 個動作
